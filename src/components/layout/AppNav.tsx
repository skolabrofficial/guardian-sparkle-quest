...
export default function AppNav() {
  ...
  return (
    <nav className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 my-6 mx-1.5 animate-float-in" style={{ animationDelay: '0.1s' }}>
      {navItems
        ...
        .map((item) => (
          <Link
            key={item.href}
            to={item.href}
            className={`sign-card ${item.color} ${location.pathname === item.href ? 'active' : ''}`}
          >
            {item.label}
          </Link>
        ))}
