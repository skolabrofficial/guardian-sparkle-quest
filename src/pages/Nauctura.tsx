        ...
        {(Object.keys(TAB_LABELS) as Tab[]).map(k => {
          ...
          return (
            <button key={k} onClick={() => setTab(k)}
              className={`px-4 py-2 rounded-full border-2 text-xs font-bold uppercase tracking-wider ${tab === k ? 'bg-foreground text-background border-foreground' : 'bg-card border-border hover:border-foreground/40'}`}>
      ...
      <DialogContent className="max-w-xl">
        ...
        <div className="grid gap-3">
          ...
          <div>
            ...
            <Textarea value={perex} onChange={e => setPerex(e.target.value)} rows={3} placeholder="Pár vět, které článek uvedou…" />
          </div>
          <div className="rounded-lg border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
            <strong>Pracovní postup:</strong> Rozpracovaný → odeslat k posouzení → redakce přebírá → úpravy a doladění → vydání (ihned, naplánované nebo významné s automatickým připnutím na titulku).
          </div>
        </div>
        <DialogFooter>
